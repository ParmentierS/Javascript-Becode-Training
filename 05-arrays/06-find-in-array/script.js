...
(() => {
    ...
    const checkObject= function(currentObject, index, array)
    {
        //console.log(currentObject, index, array);
        return currentObject.firstname==="Jean" && currentObject.lastname==="Dupont";
    }
    const findelement = function()
    {
        const bestPerson = people.find(checkObject);
        console.log("mail :",bestPerson.email);
        const bestIndex = people.findIndex(checkObject);
        console.log("Indice :", bestIndex);
    }

    const bouton = document.getElementById("run");
    bouton.addEventListener("click",findelement);
})();
